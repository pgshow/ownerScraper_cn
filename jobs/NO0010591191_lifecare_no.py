...
class SubPlugin(pluginBase.Plugin):
    ...
    def crawl(self, item):
        try:
            html = fc.fetch(url='https://lifecare.no/investor-relations/')
            ...
            # get the holders
            data_list1 = []
            for idx, tr in enumerate(soup.select('div.top-tab-content-row')):
                tds = tr.find_all('div')
                ...
                data_list1.append({
                    ...
                    'Citizen': tds[3].contents[0].strip(),
                })
            ...
            update_str = fc.update(item[1], html, "div.top-updated")
