...
class SubPlugin(pluginBase.Plugin):
    ...
    def crawl(self, item):
        try:
            html = fc.fetch(url='https://www.siemoffshore.com/investors')
            ...
            # get the holders
            data_list1 = []
            for idx, tr in enumerate(soup.select('#shareholders > div > div.w-dyn-list div[role="listitem"]')):
                tds = tr.find_all('div')
            ...
            update_str = fc.update(item[1], html, "//div[contains(text(), 'This is the list of')]")
